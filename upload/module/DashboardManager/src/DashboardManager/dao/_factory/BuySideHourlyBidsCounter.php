...
//        );
    ...
    public function getAverage($where_params = null){
    ...
    public function getAverageAdmin($where_params = null){
        
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        $select->from('buySideHourlyBidsAvarageAdmin');
    ...
    public function getAverageHeader(){
     
        $metadata = new Metadata($this->adapter);
        return $metadata->getColumnNames('buySideHourlyBidsAvarage');
...
//                    'BuySidePartnerID',
//                    'avg_bids' => new \Zend\Db\Sql\Expression('AVG(' . $this->table  . '.BidsCounter)'),
//                    'total_bids' => new \Zend\Db\Sql\Expression('SUM(' . $this->table  . '.BidsCounter)'),
//                ));
//
//                $select->join(
//                     'AdCampaignBanner',
...
//                $select->group($this->table . '.BuySidePartnerID');
//
//                $select->order($this->table . '.AdCampaignBannerID');
//
//            }
