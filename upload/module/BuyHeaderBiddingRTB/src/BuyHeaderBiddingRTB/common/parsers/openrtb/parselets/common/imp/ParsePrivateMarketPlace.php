...
class ParsePrivateMarketPlace {
	...
	public static function execute(&$Logger, \buyrtbheaderbidding\parsers\openrtb\OpenRTBParser &$Parser, \model\openrtb\RtbBidRequest &$RtbBidRequest, \model\openrtb\RtbBidRequestPmp &$RtbBidRequestPmp, &$pmp) {
		...
		if (isset($pmp["deals"]) && is_array($pmp["deals"])):
			...
			foreach ($pmp_deals_list as $pmp_deal):
				...
				try {
					...
					\buyrtbheaderbidding\parsers\openrtb\parselets\common\imp\ParseDirectDeals::execute($Logger, $Parser, $RtbBidRequest, $RtbBidRequestDirectDeals, $pmp_deal);
