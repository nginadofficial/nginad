...
class ParseUser {
	...
	public static function execute(&$Logger, \buyrtbheaderbidding\parsers\openrtb\OpenRTBParser &$Parser, \model\openrtb\RtbBidRequest &$RtbBidRequest, \model\openrtb\RtbBidRequestUser &$RtbBidRequestUser, &$rtb_user) {
		...
		*/
		...
		if (isset($rtb_user["geo"])):
			...
			\buyrtbheaderbidding\parsers\openrtb\parselets\common\ParseGeo::execute($Logger, $Parser, $RtbBidRequest, $RtbBidRequestGeo, $geo);
		...
		if (isset($rtb_user["data"]) && is_array($rtb_user["data"])):
			...
			foreach ($user_data_list as $user_data):
				...
				\buyrtbheaderbidding\parsers\openrtb\parselets\common\ParseData::execute($Logger, $Parser, $RtbBidRequest, $RtbBidRequestData, $user_data);
