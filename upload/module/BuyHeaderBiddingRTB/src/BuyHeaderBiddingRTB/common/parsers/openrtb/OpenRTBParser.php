...
namespace buyrtbheaderbidding\parsers\openrtb;
use \Exception;
...
class OpenRTBParser {
	...
	public function parse_request($config, $is_local, $rtb_ssp_friendly_name, $raw_post = null) {
		...
		// Initialize Data
		try {
			\buyrtbheaderbidding\parsers\openrtb\parselets\common\Init::execute($logger, $this, $this->RtbBidRequest);
		} catch (Exception $e) {	
        ...
        // Parse Currency
        /*
         * cur is required by OpenRTB, but HeaderBidding has USD only so they do not send it.
        try {
        	\buyrtbheaderbidding\parsers\openrtb\parselets\common\ParseCurrency::execute($logger, $this, $this->RtbBidRequest);
        } catch (Exception $e) {
        	throw new Exception($e->getMessage(), $e->getCode(), $e->getPrevious());
        }
        */
        ...
        if (isset($this->json_post["site"])):
	        ...
	        try {
	        	\buyrtbheaderbidding\parsers\openrtb\parselets\common\ParseWebsite::execute($logger, $this, $this->RtbBidRequest, $RtbBidRequestSite, $ad_campaign_site);
        ...
        if (isset($this->json_post["app"])):
	        ...
	        try {
	        	\buyrtbheaderbidding\parsers\openrtb\parselets\common\ParseApp::execute($logger, $this, $this->RtbBidRequest, $RtbBidRequestApp, $rtb_app);
        ...
        if (isset($this->json_post["user"])):
	        ...
	        try {
	        	\buyrtbheaderbidding\parsers\openrtb\parselets\common\ParseUser::execute($logger, $this, $this->RtbBidRequest, $RtbBidRequestUser, $rtb_user);
        ...
        if (isset($this->json_post["regs"])):
	        ...
	        try {
	        	\buyrtbheaderbidding\parsers\openrtb\parselets\common\ParseRegs::execute($logger, $this, $this->RtbBidRequest, $RtbBidRequestRegulations, $ad_regs);
        ...
        foreach ($ad_impression_list as $ad_impression):
			...
			if (isset($ad_impression["pmp"])):
				...
				try {
					\buyrtbheaderbidding\parsers\openrtb\parselets\common\imp\ParsePrivateMarketPlace::execute($logger, $this, $this->RtbBidRequest, $RtbBidRequestPmp, $pmp);
