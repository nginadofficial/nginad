...
class DisplayWorkflow
{
    public function process_business_rules_workflow(&$Logger, &$ParentWorkflow, \model\openrtb\RtbBidRequest &$RtbBidRequest, &$RtbBidRequestImp, &$InsertionOrderLineItem, &$InsertionOrderLineItemRestrictionsFactory) {
    	...
    	if (\buyrtbheaderbidding\workflows\tasklets\display\insertionorderlineitem\CheckIsMobile::execute($Logger, $ParentWorkflow, $RtbBidRequest, $RtbBidRequestImp, $InsertionOrderLineItem) === false):
    		return false;
    	endif;
    	 
    	// Check banner restrictions
    	if (\buyrtbheaderbidding\workflows\tasklets\display\insertionorderlineitem\BannerRestrictionsWorkflow::execute($Logger, $ParentWorkflow, $RtbBidRequest, $RtbBidRequestImp, $InsertionOrderLineItem, $InsertionOrderLineItemRestrictionsFactory) === false):
    		return false;
    	endif;
