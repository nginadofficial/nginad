...
var NginAdAdapter = require('./adapters/nginad');
...
var AolAdapter = function AolAdapter() {
	...
	function _addBid(response, context) {
		...
		bidmanager.addBidResponse(bid.placementCode, nginadBidderIdMap[bid.placementCode], bidResponse);
	}
		...
		adloader.loadScript(scriptUrl, function(response) {
			...
			bids.forEach(function(existingBid) {
				if (existingBid.params.nid === bid.params.nid) {
					...
					bidmanager.addBidResponse(existingBid.placementCode, nginadBidderIdMap[bid.params.nid], adResponse);
				}
			});
		});
	}
...
var IndexExchangeAdapter = function IndexExchangeAdapter() {
	...
	function logErrorBidResponse() {
		...
		bidmanager.addBidResponse(firstAdUnitCode, nginadBidderIdMap[firstAdUnitCode], bid);
	}
				...
				POX.addHook(function(response) {
					...
					for (i = 0; i < bids.length; i++) {
						...
						if (adUnit.get('pub_rev')) {
							...
							bidmanager.addBidResponse(bid.placementCode, nginadBidderIdMap[bid.placementCode], adResponse);
						} else {
							// Indicate an ad was not returned
							adResponse = bidfactory.createBid(2);
							adResponse.bidderCode = 'openx';
	...
	pbjs.handlePubmaticCallback = function(response) {
		...
		for (i = 0; i < bids.length; i++) {
			...
			if (adUnitInfo.bidstatus === '1') {
				...
				bidmanager.addBidResponse(bids[i].placementCode, nginadBidderIdMap[bid.adSlot], adResponse);
			} else {
				// Indicate an ad was not returned
				adResponse = bidfactory.createBid(2);
				adResponse.bidderCode = 'pubmatic';
...
var PulsePointAdapter = function PulsePointAdapter() {
    ...
    function bidResponseAvailable(bidRequest, bidResponse) {
        if(bidResponse) {
            ...
            bid.width = adSize[0];
            bid.height = adSize[1];
            bidmanager.addBidResponse(bidRequest.placementCode, nginadBidderIdMap[bidRequest.placementCode], bid);
        } else {
            var passback = bidfactory.createBid(2);
            passback.bidderCode = bidRequest.bidder;
            bidmanager.addBidResponse(bidRequest.placementCode, nginadBidderIdMap[bidRequest.placementCode], passback);
        }
    }
...
},{"../adloader":13,"../bidfactory.js":14,"../bidmanager.js":15,"../constants.json":16,"../utils.js":20}],11:[function(require,module,exports){
var CONSTANTS = require('../constants.json');
var utils = require('../utils.js');
var bidfactory = require('../bidfactory.js');
var bidmanager = require('../bidmanager.js');
    ...
    var ybotlib = {
        ...
        handleUpdateState: function() {
            ...
            utils._each(ybotlib.definedSlots, function(v) {
                ...
                bidmanager.addBidResponse(placementCode, nginadBidderIdMap[slot], bid);
