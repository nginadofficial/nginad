...
CREATE VIEW `PrivateExchangeRtbChannelDailyStatsRollUp` AS select `percds`.`PublisherWebsiteID` AS `PublisherWebsiteID`, `au`.`user_id` AS `UserID`, `au`.`parent_id` AS `ParentID`, `percds`.`MDY` AS `MDY`, `pw`.`WebDomain` AS `WebDomain`, `pw`.`IABCategory` AS `IABCategory`, `pi`.`Name` AS `PublisherName`, sum(`percds`.`ImpressionsOfferedCounter`) AS `ImpressionsOfferedCounter`, sum(`percds`.`AuctionBidsCounter`) AS `AuctionBidsCounter` from `PrivateExchangeRtbChannelDailyStats` `percds` join `PublisherWebsite` `pw` on `percds`.`PublisherWebsiteID` = `pw`.`PublisherWebsiteID` join `PublisherInfo` `pi` on `pi`.`PublisherInfoID` = `pw`.`DomainOwnerID` join `auth_Users` `au` on `au`.`PublisherInfoID` = `pi`.`PublisherInfoID` group by `percds`.`MDY`, `percds`.`PublisherWebsiteID` order by `ImpressionsOfferedCounter` ;
